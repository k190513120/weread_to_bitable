...
import { FeishuClient, parseBitableUrl, validateSyncParams } from '../api/feishu/client';
import { SyncParams } from '../config/types';

/**
 * 解析命令行参数
 */
function parseCommandLineArgs(): { bitable_url: string; personal_base_token: string; weread_cookie: string } {
...
async function main() {
  try {
    ...
    console.log('\n测试飞书多维表格连接...');
    const isConnected = await feishuClient.testConnection();
    if (!isConnected) {
    ...
    // 获取现有字段
    console.log('\n获取现有字段...');
    const existingFields = await feishuClient.getTableFields();
    ...
    for (const field of REQUIRED_FIELDS) {
      ...
      try {
        ...
        await feishuClient.createField(field);
    ...
    // 再次检查字段
    console.log('\n验证字段创建结果...');
    const updatedFields = await feishuClient.getTableFields();
