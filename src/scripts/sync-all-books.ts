...
async function main() {
  try {
    ...
    // 获取微信读书书籍列表
    console.log('\n获取微信读书书籍列表...');
    const refreshedCookie = await refreshSession(syncParams.weread_cookie);
    
    // 获取笔记本书籍（有笔记的书籍优先同步）
    const notebookBooks = await getNotebookBooks(refreshedCookie);
    console.log(`获取到 ${notebookBooks.length} 本笔记本书籍`);
    
    // 获取书架书籍
    const bookshelfBooks = await getBookshelfBooks(refreshedCookie);
    ...
    // 提取书籍ID列表
    const bookIds: string[] = uniqueBooks.map(book => book.bookId).filter(Boolean);
    console.log(`有效书籍ID数量: ${bookIds.length}`);
    
    // 执行批量同步
    console.log('\n开始执行批量同步...');
    const result = await batchSyncBooksToFeishu(
      feishuConfig,
      refreshedCookie,
      bookIds,
      true // 使用增量同步
